...
import { CADASTRAL_POLYGONS } from '../data/mockHotspots';
...
export const TacticalMapView: React.FC<TacticalMapViewProps> = ({
  ...
  const [popupHotspot, setPopupHotspot] = useState<ThermalHotspot | null>(null);

  useEffect(() => {
    if (hotspots.length > 0 && !selectedHotspot) {
      setSelectedHotspot(hotspots[0]);
      setPopupHotspot(hotspots[0]);
    }
    ...
    <div className="w-full h-[calc(100vh-73px)] flex flex-col bg-slate-900 text-slate-100 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative overflow-hidden">
        ...
        <div className="w-full lg:w-[440px] bg-white border-l border-slate-200 flex flex-col overflow-y-auto text-slate-900">
          {!selectedHotspot ? (
            <div className="p-8 flex items-center justify-center h-full text-slate-500 text-sm font-mono">
              Select a thermal anomaly on the map to inspect telemetry.
            </div>
          ) : (
            <>
              {/* Header */}
